...
export default function TurnScreen() {
  ...
  function handleConfirm() {
    if (result) setRecommendation('turn', result);
    if (actionFaced) setStreetAction('turn', actionFaced, potSize);
    setShowModal(false);
    clear();
    advanceStreet();
  }
  ...
  return (
    <ScreenWrapper title="Turn">
      <button className="back-btn" onClick={goBack}>← Back</button>

      <div className="section-label">Turn Card</div>
      <CardPicker
        label="Turn"
        existingCard={boardCards[3]}
        usedCards={allCards}
        onCardSelected={(c) => setBoardCard(3, c)}
      />
